...
class BaseModel(Model):
    ...
    def find_one(cls, db, query, collection=None, model=True):
        ...
        for i in cls.reconnect_amount():
            try:
                result = yield db[cls.check_collection(collection)].find_one(query)
    ...
    @gen.coroutine
    def update(self, db=None, query=None,
               collection=None, update=None, ser=None,
        ...
        """
        db = db or self.db
        # TODO: refactor, update and ser arguments are very similar, left only
        # one
        c = self.check_collection(collection)
        if update is None:
            data = self.get_data_for_save(ser)
        ...
        for i in self.reconnect_amount():
            try:
                result = yield db[c].update(query, data,
                                            upsert=upsert,
                                            multi=multi)
            except ConnectionFailure as e:
...
def test():
    ...
    news = NewsModel(nm)
    #yield news.save(db)
    res = yield NewsModel.find_one(db, {"title": "title"})
    print(res.title)
    print(news.title)
